...
func main() {
    ...
    init_main() // TODO: some better way?
    ...
    var app = App(game)
    app.build_cache()
    print(app.ev_cache[Int(game.id)])  // # starting game state, should have expected value of 255.5896
}
...
let Y="Y"; let N="N"; let S="S"; let D="D"; let C=",";
func print(state :GameState , choice_ev: ChoiceEV ) { 
    ...
    if (state.rolls_remaining==0){ // for slot choice 
        ...
        sb+=(D); sb+=(C);
        var tmp = choice_ev.choice.description
        tmp += tmp.padding(toLength: 5, withPad: "0", startingAt: sb.count)
        sb += tmp.suffix(5)
    }
    sb+=(C);
...
struct Slots : Collection  {
    ...
    init(_ array:[Slot]){  
        var mask:u16 
        for slot in array {
            mask = u16( Slot(0x0001) << slot)
            self.data |= mask; // force on
        } 
    }

    // //implement Sequence protocol
    // typealias Iterator = SlotsIterator
    // func makeIterator() -> SlotsIterator { return SlotsIterator(self) }
    ...
    subscript(position: Int) -> Slot {
        get { 
            ...
            var bit_index=0
            var j=2
            while (j <= position){ //the slots.data does not use the rightmost (0th) bit 
...
// struct SlotsIterator : IteratorProtocol {
//     var slots:Slots
//     var i:Int = 0
//     init(_ slots:Slots) { self.slots = slots }
//     mutating func next() -> Slot? {
//         if i < slots.endIndex { defer { i += 1 }; return slots[i] }
//         return nil
//     }
// }
...
struct App{
    ...
    public var ev_cache :[ChoiceEV] 
    public var bar :Tqdm

    // return a newly initialized app
    init(_ game :GameState) {
        let ticks = game.counts()
        self.game = game
        let bar = Tqdm(total:ticks)
        self.bar = bar
    ...
    func output(state :GameState, choice_ev :ChoiceEV ){ 
        ...
        print(state:state, choice_ev:choice_ev)
    } 
    ...
    public mutating func build_cache() {
    ...
    mutating func process_dieval_combo(_ rolls_remaining :u8, _ slots_len :Int, _ slots :Slots, _ dieval_combo :DieVals, _ joker_rules_in_play :Bool, 
